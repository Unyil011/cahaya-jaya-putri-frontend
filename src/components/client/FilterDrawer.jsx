            ...
            <div className="p-4 md:p-6 flex-1 overflow-y-auto">
              ...
              <div className="mb-6">
                ...
                <div className="space-y-1">
                  {['Semua', 'Lunas', 'Menunggu Konfirmasi', 'Belum Lunas'].map(status => (
                    <label
                      key={status}
                      className="flex items-center gap-2 py-1 cursor-pointer transition-colors hover:text-mbg-blue-600 dark:hover:text-mbg-blue-400"
                    >
                      <input 
                        type="radio" 
                        name="payment_status"
                        ...
                        className="w-4 h-4 text-mbg-blue-500 bg-white border-gray-300 focus:ring-mbg-blue-500 dark:focus:ring-mbg-blue-600 dark:ring-offset-gray-800 dark:bg-slate-700 dark:border-slate-600"
                      />
                      <span className={`text-sm ${filterPayment === status ? 'font-semibold text-mbg-blue-600 dark:text-mbg-blue-400' : 'text-gray-700 dark:text-gray-300'}`}>{status}</span>
                    </label>
                  ))}
                </div>
              </div>
              ...
              <div className="mb-6">
                  ...
                  {['Semua Waktu', 'Mingguan', 'Bulanan', 'Tahunan'].map(type => (
                    ...
                    >
                      <input 
                        ...
                        className="w-4 h-4 text-mbg-blue-500 bg-white border-gray-300 focus:ring-mbg-blue-500 dark:focus:ring-mbg-blue-600 dark:ring-offset-gray-800 dark:bg-slate-700 dark:border-slate-600"
                      />
                      <span className={`text-sm ${((type === 'Semua Waktu' && filterTime.type === '') || filterTime.type === type) ? 'font-semibold text-mbg-blue-600 dark:text-mbg-blue-400' : 'text-gray-700 dark:text-gray-300'}`}>{type}</span>
                    </label>
                  ))}
                </div>
                ...
                {filterTime.type === 'Mingguan' && (
                  <div className="mt-3 pl-6">
                    ...
                    <input type="date" value={filterTime.value} onChange={e => setFilterTime({...filterTime, value: e.target.value})} className="w-full px-3 py-1.5 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg text-sm focus:outline-none focus:ring-1 focus:ring-mbg-blue-500" />
                  </div>
                )}
                {filterTime.type === 'Bulanan' && (
                  <div className="mt-3 pl-6">
                    <label className="block text-xs font-medium text-gray-500 mb-1">Pilih Bulan & Tahun</label>
                    <input type="month" value={filterTime.value} onChange={e => setFilterTime({...filterTime, value: e.target.value})} className="w-full px-3 py-1.5 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg text-sm focus:outline-none focus:ring-1 focus:ring-mbg-blue-500" />
                  </div>
                )}
                {filterTime.type === 'Tahunan' && (
                  <div className="mt-3 pl-6">
                    <label className="block text-xs font-medium text-gray-500 mb-1">Pilih Tahun</label>
                    <select value={filterTime.value} onChange={e => setFilterTime({...filterTime, value: e.target.value})} className="w-full px-3 py-1.5 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg text-sm focus:outline-none focus:ring-1 focus:ring-mbg-blue-500">
